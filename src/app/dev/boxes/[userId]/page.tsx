...
export default function UserBoxPage() {
  ...
  const fetchFiles = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/dev/boxes/${userId}`);
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const res = await fetch(`/api/dev/boxes/${userId}/upload`, {
        method: "POST",
        body: formData,
      });
      const data = await res.json();
  ...
  const quotaPct = 0; // TODO: quota par utilisateur

  return (
    <div className="p-8 max-w-5xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white">Ma Box</h1>
        <p className="text-cyan-400 text-sm mt-1">{username}</p>
      </div>

      <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-5">
        <div className="flex items-center justify-between text-sm mb-2">
          <span className="text-neutral-400">Stockage utilisé</span>
          <span className="text-white font-medium">{formatSize(quotaUsed * 1024 * 1024)}</span>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-8 text-center">
          <p className="text-neutral-400">Votre box est vide.</p>
          <p className="mt-1 text-sm text-neutral-500">Ajoutez des fichiers pour commencer.</p>
        </div>
      ) : (
